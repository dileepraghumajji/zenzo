...
/** Derive URL slug from studio name */
...
export default function OnboardingPage() {
  ...
  const [initLoading, setInitLoading] = useState(true); // checking existing session
  ...
  // Tenant (set after step 2 completes)
  ...
  // ── On mount: redirect if user already completed onboarding ──────────────────
  ...
  async function handleStep2Next() {
    ...
    // If tenant is already created (user went back and re-submitted), just advance
    if (tenantId) {
      setStep(3);
      return;
    }
    ...
    try {
      ...
      const json = (await res.json()) as { tenantSlug: string; tenantId: string };
      setTenantId(json.tenantId);
      setTenantSlug(json.tenantSlug);
      setStep(3);
    } catch {
